...
export default function HeroProductMotion() {
  ...
  return <div ref={host} className="hero-products-visual relative min-w-0 bg-transparent md:h-[332.5px] md:overflow-hidden">
    {mobile ? <MobileAcousticSequence motion={motion && visible} /> : motion && visible ? <video ref={video} autoPlay loop muted playsInline controls={false} disablePictureInPicture preload="auto" poster={poster} aria-label="Produtos Sonar e propagação de ondas sonoras" className="hero-products-video pointer-events-none absolute inset-0 h-full w-full select-none object-contain">
      <source src="/media/hero-products-transparent.webm" type="video/webm" /><source src="/media/hero-products-cropped.mp4" type="video/mp4" />
    </video> : <img src={poster} width={900} height={470} fetchPriority="high" alt="Produtos Sonar demonstrando absorção, difusão, atenuação e bloqueio das ondas sonoras" className="hero-products-image pointer-events-none absolute inset-0 h-full w-full select-none object-contain" />}
