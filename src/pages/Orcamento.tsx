...
export default function OrcamentoPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // A função busca os dados diretamente no registro recém-criado. Assim,
      // o navegador envia apenas o protocolo e não consegue adulterar o
      // destinatário nem o conteúdo usado nos e-mails.
      const { error: emailError } = await supabase.functions.invoke("quote-request", {
        body: { requestId },
      });
      ...
      if (!emailError) {
        ...
        console.warn("Quote registered but confirmation email failed", emailError);
